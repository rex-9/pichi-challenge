...
const Woolf = ({ woolf, onClick, selected, stats }) => {
  ...
  //const MAXIMUM_GLOBAL_WOOL = toBigInt(24000000).mul(toBigInt(10).pow(toBigInt(18)))
  const MAXIMUM_GLOBAL_WOOL = toBigInt(2400000000) * toBigInt(10) ** toBigInt(9)
  ...
  const unclaimedWool = () => {
    ...
    if (woolf.isSheep) {
      ...
      if (toBigInt(woolEarned).gte(MAXIMUM_GLOBAL_WOOL)) duration = Math.max(stats.lastClaimTimestamp - woolf.value, 0)
      const earnings = parseUnits('10000', 'gwei') * toBigInt(duration) / toBigInt(24 * 60 * 60)
      return parseBigNumber(earnings, 0)
    } else {
      const woolPerAlpha = stats ? stats.woolPerAlpha : woolf.value
      const earnings = (toBigInt(woolPerAlpha) - toBigInt(woolf.value)) * toBigInt(woolf.alpha)
      return parseBigNumber(earnings, 0)
    }
  }
